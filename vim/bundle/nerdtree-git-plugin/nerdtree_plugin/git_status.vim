...
function! AirlineDevIconsGitGetCurrFileIndicator()
    let gitcmd = 'git -c color.status=false status -s ' . expand('%:p')
    let file_status = system(gitcmd)
    let status_key = s:NERDTreeGetFileGitStatusKey(file_status[0], file_status[1])
    if status_key ==# 'Unknown'
        return ''
    endif
    return s:NERDTreeGetIndicator(status_key)
endfunction

function! AirlineDevIconsGit(...)
    let w:airline_section_x = get(w:, 'airline_section_x',
                \ get(g:, 'airline_section_x', ''))
    let w:airline_section_x = ' %{AirlineDevIconsGitGetCurrFileIndicator()} ' . w:airline_section_x
endfunction

" TODO: Airline fucks vim if running a 'system()' command from within a hook.
"       Need to somehow cache the statuses..
" call airline#add_statusline_func('AirlineDevIconsGit')
