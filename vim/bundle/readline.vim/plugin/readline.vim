...
function! s:upcase_word()
  let x = s:getcur()
  let y = s:next_word(x)
  return repeat("\<del>", y - x) . substitute(toupper(strpart(
...
function! s:downcase_word()
  let x = s:getcur()
  let y = s:next_word(x)
  return repeat("\<del>", y - x) . substitute(tolower(strpart(
  \ getcmdline(), x, y - x)), '[[:cntrl:]]', "\<c-v>&", 'g')
endfunction
...
function! s:capitalize_word()
  ...
  while x < y
    ...
    let x += 1
    if c =~ s:wordchars
      let cmd .= "\<del>" . toupper(strpart(s, x - 1, 1))
      break
    else
      let cmd .= "\<right>"
    endif
  endwhile
  let cmd .= repeat("\<del>", y - x) . substitute(tolower(strpart(
  \ getcmdline(), x, y - x)), '[[:cntrl:]]', "\<c-v>&", 'g')
...
function! s:transpose_chars()
  ...
  \ substitute(strpart(s, x - 1, 1), '[[:cntrl:]]', "\<c-v>&", '')
endfunction
...
function! s:transpose_words()
  ...
  let str1 = strpart(s, beg1, end1 - beg1)
  let str2 = strpart(s, beg2, end2 - beg2)
...
function! s:delete_to(x, y)
  ...
  if a:y < a:x
    let s:yankbuf = strpart(getcmdline(), a:y, a:x - a:y)
    return repeat("\<del>", a:x - a:y)
  endif
  let s:yankbuf = strpart(getcmdline(), a:x, a:y - a:x)
...
function! s:prev_word(x)
  let s = getcmdline()
  let x = a:x
  while x > 0 && strpart(s, x - 1, 1) !~ s:wordchars
    let x -= 1
  endwhile
  while x > 0 && strpart(s, x - 1, 1) =~ s:wordchars
    let x -= 1
...
function! s:next_word(x)
  ...
  let n = strchars(s)
  let x = a:x
  while x < n && strpart(s, x, 1) !~ s:wordchars
    let x += 1
  endwhile
  while x < n && strpart(s, x, 1) =~ s:wordchars
    let x += 1
